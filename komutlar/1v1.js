...
exports.run = async (client, message, args) => {
  ...
  if (!opponent)
    return message.reply(
      " :name_badge: | Oynamak istediğin kişiyi etiketlemelisin! "
    );

  if (opponent.bot)
    return message.reply(":name_badge:  | Botlar ile oynayamazsın!");
  if (opponent.id === message.author.id)
    return message.reply(":name_badge:  | Kendin ile düello atamazsın!");
  if (this.fighting.has(message.channel.id))
    return message.reply(
      ":name_badge:  | Kanal başına sadece bir düello meydana gelebilir."
    );
